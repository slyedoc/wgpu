...
fn query_cooperative_matrix_properties(
    ...
    // Two-call idiom in newer ash: first query the count, then allocate
    // a buffer of that size and fill it on the second call.
    let count = match unsafe {
        coop_matrix.get_physical_device_cooperative_matrix_properties_len(phd)
    } {
        Ok(c) => c,
        Err(e) => {
            log::warn!("Failed to query cooperative matrix property count: {e:?}");
            return Vec::new();
        }
    };
    let mut vk_properties: Vec<vk::CooperativeMatrixPropertiesKHR<'_>> =
